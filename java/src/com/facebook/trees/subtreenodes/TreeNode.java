package com.facebook.trees.subtreenodes;
...
class TreeNode {
    ...
    int[] countOfNodes(Node root, ArrayList<Query> queries, String s) {
        ...
        Map<Integer, Character> mapChar = new HashMap<>();
        Queue<Node> queue = new LinkedList();
        queue.add(root);
        while (!queue.isEmpty()) {
            Node next = queue.remove();
            mapNode.put(next.val, next);
            mapChar.put(next.val, s.charAt(next.val -1));
            queue.addAll(next.children);
        }

        // Process the query
        int[] resp = new int[queries.size()];
        for (int i=0; i< queries.size(); i++) {
            Node startNode = mapNode.get(queries.get(i).u);
            char search = queries.get(i).c;

            // Traverse tree and count
            Queue<Node> queueNum = new LinkedList();
            queueNum.add(startNode);
            while (!queueNum.isEmpty()) {
                Node next = queueNum.remove();
                if (mapChar.get(next.val) == search) {
                    resp[i]++;
                }
                queueNum.addAll(next.children);
            }
        }
        return resp;
    }
