package com.facebook.linkedlist.reverse;
...
class Main {
    ...
    // Return the first node to process after reversing
    Node reverse(Node currNode, Node prevNode) {

        // Reverse till we find even element
        Node last_temp, head_temp;
        Node headNode = prevNode;

        //System.out.println("\nProcessing list:");
        //printLinkedList(headNode);
        //System.out.println("\n---");
        while (currNode.next != null && currNode.next.data % 2 == 0 ) {
            last_temp = currNode.next.next;
            head_temp = headNode.next;

            headNode.next = currNode.next;
            currNode.next.next = head_temp;
            currNode.next = last_temp;
    ...
    Node reverse(Node head) {
        ...
        headStart.next = head;
        Node curNode = head;
        Node prevNode = headStart;
        while (curNode != null) {
            if (curNode.data % 2 == 0) {
                curNode = reverse(curNode, prevNode);
                prevNode = curNode;
                // System.out.println("\nUpdated list--");
